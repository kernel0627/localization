...
当前已完成第一问的定位、匿名发射机辨识与圆形编队调整；第二问的二维静态几何 E0、三角基准校准、三参考并行定位与理想坐标闭环已完成本轮验证，详见[三角基准调整说明](docs/q2/三角基准调整.md)。

第二问已补[参考残差传播与有限校准阈值](docs/q2/参考残差传播与校准阈值.md)，给出 FY01 校准精度对其余 12 架的影响，以及相同初态下精度和完整测角成本的对照。
...
| 2（三角基准闭环） | FY11/FY15 固定基准、FY01 bootstrap、ABC 三发射和 12 点并行定位；理想无噪声闭环批次 | [调整说明](docs/q2/三角基准调整.md) |
