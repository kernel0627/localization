# Localization

2022 年高教社杯全国大学生数学建模竞赛 B 题“无人机遂行编队飞行中的纯方位无源定位”的解题工作底稿，汇集模型、计算过程、实验结果和论文图表。

当前已完成第一问的定位、匿名发射机辨识与圆形编队调整；第二问已完成参考建立、并行定位、有限校准、几何边界、含噪增益及协议成本分析，见[第二问整体方法](solutions/q2/README.md)。正文按参考建立、夹角定位、并行反馈和停止展开，配套图表及数据入口见[第二问资料索引](docs/q2/资料索引.md)。

## 阅读入口

先读[题目](problem/B题.md)，再读[第一问整体解法](solutions/q1/README.md)。整体解法按三个子问题连续展开，可作为撰写第一问论文的技术底稿；各分问也可单独阅读。

| 分问 | 已完成的方法与结论 | 详细解答 |
|---|---|---|
| 1（1）已知编号定位 | 用三组夹角建立非线性最小二乘模型，结合自身编号与小偏差条件选择位置解 | [定位模型](solutions/q1_1/README.md) |
| 1（2）匿名发射机辨识 | 枚举匿名编号并检查定位一致性；精确测角、偏差充分小时，FY00、FY01 之外再增加 1 架具有局部充分性 | [辨识模型](solutions/q1_2/README.md) |
| 1（3）圆形编队调整 | 四机发射、接收机估计本机径向与角向偏差并同步修正；由全 28 配置轮换改进为两配置交替 | [调整方案](solutions/q1_3/README.md) |
| 2（E0）二维形状局部可辨识性 | 15 点三角网格、静止带编号角度批次；455 个单配置与双配置见证的 Jacobian 探针 | [E0 说明](docs/q2/E0局部可辨识性.md) |
| 2（三角编队调整） | 固定底边、有限校准 FY01、三参考并行反馈；参考和测角误差预算、阶段增益、协议对照 | [整体方法](solutions/q2/README.md)、[实现说明](docs/q2/三角基准调整.md) |

1（3）的双配置为 FY00、FY01、FY04、FY05 与 FY00、FY01、FY07、FY08 交替发射。表 1 初态、精确测角与执行、同一增益 $\eta=0.5$ 下，首次达到全队最大位置误差小于 $1\,\mathrm{cm}$ 的累计测角时隙从 **92 降至 31**。一个时隙包括四机同时发射、其余六机测角和同步调整。

## 单文件复现入口

[scripts/q1/q1.py](scripts/q1/q1.py) 将 Q1.1、Q1.2 和 Q1.3 的当前方法整理为一个可独立运行的文件，不依赖 Q1.3 的历史附录。以双配置表 1 算例为例，从仓库根目录执行：

```bash
conda run -n agent python scripts/q1/q1.py q1_3_2 --no-plot --output-dir scratch/q1_reproduction/q1_3_2
```

本机使用 Conda `agent` 环境运行。这条复现会报告首次厘米级时隙、结束时隙、发射机使用次数和累计端点位移。

[scripts/q2/q2.py](scripts/q2/q2.py) 集中第二问的主方法与全部配套分析，单独复制文件即可运行。基础闭环示例：

```bash
conda run -n agent python scripts/q2/q2.py triangle --output-dir scratch/q2_reproduction/triangle
```

协议对照用 `protocols`，测角噪声与增益用 `noise`，完整命令和函数索引见[第二问代码说明](scripts/q2/README.md)。七个子命令已完成隔离复现，467 个输出与原模块版一致。

## 写论文时取用什么

- **第二问解法与图表**：[编队调整方案](solutions/q2/README.md)、[资料索引](docs/q2/资料索引.md)。正文包含两阶段方法、同一轨迹实例和校准规则对照三张图；详细依据见[技术底稿](docs/q2/方法推导与模型检验.md)。
- **模型与公式**：[第一问整体解法](solutions/q1/README.md)。1.3 按一次调整、轮换互校、配置选择、仿真结果和模型检验展开。
- **图表与原始数据**：[1.3 资料索引](docs/q1_3/资料索引.md) 按论文内容列出对应图、表和数据；1.1、1.2 的数据分别见[定位验证记录](docs/q1_1/验证记录.md)和[辨识验证记录](docs/q1_2/验证记录.md)。
- **计算与复现**：[1.3 代码入口](scripts/q1_3/README.md) 列出仿真、出图和分析命令，以及各命令的依赖。

## 目录与运行

| 目录 | 内容 |
|---|---|
| `problem/` | 题面及原图 |
| `solutions/` | 整体解法与各分问正文 |
| `docs/` | 详细推导、模型检验和计算记录 |
| `figures/`、`outputs/` | 论文图片、汇总表和逐次实验数据 |
| `scripts/`、`tests/` | 算法、仿真与实现检查 |

在仓库根目录使用 Conda `agent` 环境。已有数据可以直接查阅；以下命令将表 1 的全配置调整另存到本地结果目录：

```bash
conda run -n agent python -m pip install -r requirements.txt
conda run -n agent python -m scripts.q1_3.run_adjustment --output-dir scratch/q1_3_reproduction/main
```

全配置控制器位于 [local_adjustment.py](scripts/q1_3/local_adjustment.py)。双配置的旧模块分析入口仍依赖本地 archive，具体文件要求见[代码说明](scripts/q1_3/README.md#控制器与实验)；单文件入口可独立复现。正式目录内的正文、图表和汇总数据可直接用于阅读与论文整理。
