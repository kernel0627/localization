...
默认参数下的轨迹如下，数值取自[逐时隙误差记录](../../outputs/q1_3/error_history.csv)。
...
已有[敏感性结果](../../outputs/q1_3/sensitivity.csv)中，表 1 在增益 $0.25,\ 0.5,\ 1$ 下分别于累计 224、196、196 时隙完成停止确认，末态最大位置误差分别约为 $1.52\times10^{-4}$、$5.18\times10^{-5}$、$9.05\times10^{-6}\,\mathrm{m}$；三组额外笛卡尔扰动均于 196 时隙完成确认，末态最大误差处于 $5.70\times10^{-5}$ 至 $7.46\times10^{-5}\,\mathrm{m}$。理想初态保持零位置误差，于 56 时隙完成确认。这些有限案例均无局部拟合失败。
...
模型实现见 [local_adjustment.py](../../scripts/q1_3/local_adjustment.py)，仿真入口见 [run_adjustment.py](../../scripts/q1_3/run_adjustment.py)。求解器设置、候选有效性检查及测试记录见[验证记录](../../docs/q1_3/验证记录.md)。

主要结果包括[汇总参数与误差](../../outputs/q1_3/summary.json)、[公开轮换表](../../outputs/q1_3/transmitter_schedule.csv)、[参考候选](../../outputs/q1_3/local_candidates.csv)和[逐机调整日志](../../outputs/q1_3/adjustment_steps.csv)。完整的第一问推导见[第一问整体解法](../q1/README.md)。
